...
function updateCookieString(cookieStr) {
  ...
  let result = Object.entries(cookies).map((entry) => cookie.serialize(entry[0], entry[1], {encode: (x) => x})).join('; ');
  return result;
}
...
app.use(function (req, res) {
  ...
  let aeIdx = -1;
  for (let i = 0; i < req.rawHeaders.length; i += 2) {
    if (req.rawHeaders[i].toLowerCase() == 'accept-encoding') {
      aeIdx = i + 1;
      break;
    }
  }
  ...
  if (aeIdx != -1) {
    let value = (req.rawHeaders[aeIdx] ?? "");
    if (value.includes("deflate") || value.includes("gzip"))
    {
       req.rawHeaders[aeIdx] = "";
    }
  }
...
proxy.on('proxyReqWs', (proxyReq, req, socket, options, head) => {
  ...
  console.log("WS >>> : ", proxyReq.method, " ", proxyReq.url);
});

proxy.on('proxyRes', function (proxyRes, req, res) {
  var cookies = res.getHeader('Set-Cookie');
  if (!cookies) {
    // XXX: HP iLO4, handles cookie from browser, so we need to expose it for html console to work.
    cookies = Object.entries(session.cookies).map((entry) => cookie.serialize(entry[0], entry[1], {encode: (x) => x}))
...
console.log("Proxy is listening, w/ ", session);
